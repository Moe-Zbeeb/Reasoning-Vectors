...
DEFAULT_DATA_DIR = str(EVAL_DIR / "data")
...
def parse_args():
    ...
    parser.add_argument(
        "--data_names", "--data_name", dest="data_names", default="gsm8k,math", type=str
    )
...
def get_visible_gpu_count():
    ...
    if cuda_visible_devices:
        devices = [device for device in cuda_visible_devices.split(",") if device.strip()]
        return len(devices)
...
def setup(args):
    # load model
    available_gpu_count = get_visible_gpu_count()
    sampling_params_class = None
    if args.use_vllm:
        if "mimo" in args.model_name_or_path.lower():
            __import__("register_mimo_in_vllm")
        from vllm import LLM, SamplingParams

        sampling_params_class = SamplingParams
        if available_gpu_count < 1:
            raise RuntimeError("vLLM evaluation requires at least one visible CUDA GPU.")
        if available_gpu_count % args.pipeline_parallel_size != 0:
            raise ValueError(
                f"Visible GPUs ({available_gpu_count}) must divide evenly by pipeline_parallel_size ({args.pipeline_parallel_size})."
            )
        tensor_parallel_size = available_gpu_count // args.pipeline_parallel_size
        llm_kwargs = {
    ...
    for data_name in data_list:
        results.append(main(llm, tokenizer, data_name, args, sampling_params_class))
...
def main(llm, tokenizer, data_name, args, sampling_params_class=None):
    ...
    if args.apply_chat_template and getattr(tokenizer, "chat_template", None):
        input_prompts = [
        ...
        print("Chat template unavailable; using raw prompts.")
