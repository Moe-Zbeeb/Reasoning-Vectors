...
def evaluate(
    ...
    params = [
        (idx, pred, sample["gt"]) for idx, sample in enumerate(samples) for pred in sample["pred"]
    ]
    ...
    if "type" in samples[0]:
        ...
        type_scores = {
            k: np.round(np.array(v).mean() * 100, decimals=1) for k, v in type_scores.items()
        }
